...
<div class="wrap">
	...
	<h2 class="nav-tab-wrapper">
		<a href="<?php echo admin_url( 'nav-menus.php' ); ?>" class="nav-tab<?php if ( ! isset( $_GET['action'] ) || isset( $_GET['action'] ) && 'locations' != $_GET['action'] ) echo ' nav-tab-active'; ?>">修改导航</a>
	</h2>
	<?php
	...
	<div class="manage-menus">
		...
		<span class="add-edit-menu-action">
			<?php printf( __( 'Edit your menu below,<a href="%s">.</a>' ), esc_url( add_query_arg( array( 'action' => 'edit', 'menu' => 0 ), admin_url( 'nav-menus.php' ) ) ) ); ?>
		</span><!-- /add-edit-menu-action -->
		<?php else : ?>
			<form method="get" action="<?php echo admin_url( 'nav-menus.php' ); ?>">
			<input type="hidden" name="action" value="edit" />
			<label for="menu" class="selected-menu">选择要修改的导航:</label>
			<select name="menu" id="menu">
				...
				<?php foreach( (array) $nav_menus as $_nav_menu ) : ?>
					<option value="<?php echo esc_attr( $_nav_menu->term_id ); ?>" <?php selected( $_nav_menu->term_id, $nav_menu_selected_id ); ?>>
						...
						?> (ID:<?php echo esc_attr( $_nav_menu->term_id ); ?>)
					</option>
				<?php endforeach; ?>
			</select>
			<span class="submit-btn"><input type="submit" class="button-secondary" value="<?php _e( 'Select' ); ?>"></span>
			<span class="add-new-menu-action">
				<?php printf( __( '<a style="color:#ffffff" href="%s">.</a>' ), esc_url( add_query_arg( array( 'action' => 'edit', 'menu' => 0 ), admin_url( 'nav-menus.php' ) ) ) ); ?>
					...
					<div id="post-body">
						<div id="post-body-content">
							...
							<?php if ( $add_new_screen ) : ?>
								...
								<?php endif; ?>
							<?php endif; ?>
							<div class="menu-settings" style="display: none;"<?php if ( $one_theme_location_no_menus ) { ?><?php } ?>>
								<h3>导航关联(定制网站已经做好关联，切勿自行修改！)</h3>
								<?php
								if ( ! isset( $auto_add ) ) {
								...
								<?php if ( current_theme_supports( 'menus' ) ) : ?>
									...
									<dl class="menu-theme-locations">
										...
										<dd class="checkbox-input">
											...
											<span class="theme-location-set"> <?php printf( __( "(关联到模版导航为: %s)" ), wp_get_nav_menu_object( $menu_locations[ $location ] )->name ); ?> </span>
											<?php endif; ?>
										</dd>
										<?php endforeach; ?>
									</dl>
