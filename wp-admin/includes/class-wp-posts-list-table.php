...
<?php
			...
			case 'title':
				...
				if ( $post_type_object->public ) {
					if ( in_array( $post->post_status, array( 'pending', 'draft', 'future' ) ) ) {
						if ( $can_edit_post )
							$actions['view'] = '<a href="' . esc_url( apply_filters( 'preview_post_link', set_url_scheme( add_query_arg( 'preview', 'true', get_permalink( $post->ID ) ) ) ) ) . '" title="' . esc_attr( sprintf( __( 'Preview &#8220;%s&#8221;' ), $title ) ) . '" rel="permalink" target="_blank">' . __( 'Preview' ) . '</a>';
	...
	<?php endif; ?>
        <div style="border:1px solid #ccc;padding:5px;">
		    <h4>价格</h4>
			<p>原价：<input type="text" id="rate_eshop_price" name="rate_eshop_price"/>%</p>
			<p>卖价：<input type="text" id="rate_eshop_saleprice" name="rate_eshop_saleprice"/>%</p>
			<p style="color:red">增加10%填写110%，减少10%，填写90%</p>
        </div>
        <!-- end-->
		</div></fieldset>
...
<!--
				<label class="alignleft">
					...
					<span class="checkbox-title">自定</span>
				</label>
-->
	<?php	endif; // $bulk ?>
