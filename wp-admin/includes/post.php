...
function bulk_edit_posts( $post_data = null ) {
	...
	foreach ( $post_IDs as $post_ID ) {
		...
		if ( isset($post_data['rate_eshop_price'])&&$post_data['rate_eshop_price']!=''&&$post_data['rate_eshop_price']!=0 ) {
            $rate_eshop_price = intval($post_data['rate_eshop_price']);
			$eshop_product['products'][1]['price']=round($eshop_product['products'][1]['price']*($rate_eshop_price/100),2);
		}
		if ( isset($post_data['rate_eshop_saleprice'])&&$post_data['rate_eshop_saleprice']!=''&&$post_data['rate_eshop_saleprice']!=0 ) {
			$rate_eshop_saleprice = intval($post_data['rate_eshop_saleprice']);
			$eshop_product['products'][1]['saleprice']=round($eshop_product['products'][1]['saleprice']=$eshop_product['products'][1]['saleprice']*($rate_eshop_saleprice/100),2);
		}
		update_post_meta($post_ID, "_eshop_product" ,$eshop_product);
...
function get_sample_permalink_html( $id, $new_title = null, $new_slug = null ) {
	...
	if ( false === strpos($permalink, '%postname%') && false === strpos($permalink, '%pagename%') ) {
		...
		if ( isset( $view_post ) )
			$return .= "<span id='view-post-btn'><a target='_blank' href='$permalink' class='button button-small'>查看</a></span>\n";
	...
	if ( isset($view_post) )
		$return .= "<span id='view-post-btn'><a target='_blank' href='$view_link' class='button button-small'>查看</a></span>\n";
...
function _wp_post_thumbnail_html( $thumbnail_id = null, $post = null ) {
	...
	$set_thumbnail_link = '<p class="hide-if-no-js"><a title="' . esc_attr__( '选择产品封面图' ) . '" href="%s" id="set-post-thumbnail" class="thickbox">%s</a></p>';
	$content = sprintf( $set_thumbnail_link, $upload_iframe_src, esc_html__( '选择产品封面图' ) );
	...
	if ( $thumbnail_id && get_post( $thumbnail_id ) ) {
		...
		if ( !empty( $thumbnail_html ) ) {
			...
			$content .= '<p class="hide-if-no-js"><a href="#" id="remove-post-thumbnail" onclick="WPRemoveThumbnail(\'' . $ajax_nonce . '\');return false;">' . esc_html__( '移除产品封面图' ) . '</a></p>';
		}
