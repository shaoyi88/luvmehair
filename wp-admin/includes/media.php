...
function media_buttons($editor_id = 'content') {
	...
	echo '<a href="#" id="insert-media-button" class="button insert-media add_media" data-editor="' . esc_attr( $editor_id ) . '" title="' . esc_attr__( 'Add Media' ) . '"><span class="wp-media-buttons-icon"></span> 插入图片</a>';
...
function get_media_item( $attachment_id, $args = null ) {
	...
	if ( $send )
		$send = get_submit_button( __( '点击插入' ), 'button', "send[$attachment_id]", false );
	...
	if ( ( $send || $thumbnail || $delete ) && !isset( $form_fields['buttons'] ) )
		$form_fields['buttons'] = array( 'tr' => "\t\t<tr class='submit'><td></td><td class='savesend'>$send $thumbnail $delete <b style='color:red;'>注：插入前，请确定上面 '连接Url', 如果为空, 请点击'文件 URL'按钮, 尺寸选择'完整尺寸'</b></td></tr>\n" );
...
';
	...
	<table class="describe ' . $table_class . '"><tbody>
		...
		<tr class="image-only">
			<td></td>
			<td>
				<input type="button" class="button" id="go_button" style="color:#bbb;" onclick="addExtImage.insert()" value="' . esc_attr__('点击插入') . '" />
			</td>
		</tr>
		<tr class="not-image">
			<td></td>
			<td>
				' . get_submit_button( __( '点击插入' ), 'button', 'insertonlybutton', false ) . '
			</td>
		</tr>
	</tbody></table>
';
