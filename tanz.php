...
<div id="page">
    <div id="header">
        ...
        <div id="download"> <A HREF="http://www.hkcdn.net">官方网站</A></div>
    </div>
...
{
?>		
		  Cache化内存为 <span id="CachedMemory">0</span>
		  , 使用率 
          <span id="memCachedPercent">0</span>
		  %	| Buffers缓冲为  <span id="Buffers">0</span>
...
<?php preg_match_all( "/([^\s]+):[\s]{0,}(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)\s+(\d+)/", $strs[$i], $info );?>
     <tr>
        ...
        <td width="43%">已发送 : <font color='#CC0000'><span id="NetOut<?php echo $i?>">0</span></font></td>
    </tr>
<?php endfor; ?>
</table>
<?php endif; ?>
...
<div id="footer">
&copy; <?php echo date("Y",time());?> <A HREF="http://www.hkcdn.net" target="_blank"><?php echo $title.$version;?></A> All Rights Reserved.<br />
