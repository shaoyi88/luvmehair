...
<form method="post" action="<?php echo esc_url( add_query_arg( array( 'post' => $post_id ), menu_page_url( 'wpcf7', false ) ) ); ?>" id="wpcf7-admin-form-element"<?php do_action( 'wpcf7_post_edit_form_tag' ); ?>>
	...
	<div id="titlediv">注：不熟悉HTML代码的客户，勿修改表格中的内容，请在有技术人员指导的条件下进行修改，或者查看相关教程。
	<div style="display:none;" >	<input type="text" id="wpcf7-title" name="wpcf7-title" size="40" value="<?php echo esc_attr( $post->title ); ?>"<?php echo $disabled; ?> /></div>
		<?php if ( current_user_can( 'wpcf7_edit_contact_form', $post_id ) ) : ?>
		...
		<?php endif; ?>
		<?php if ( ! $post->initial ) : ?>
		<p style="display:none;" class="tagcode">
		...
		<div  style="display:none;" class="actions-link">
