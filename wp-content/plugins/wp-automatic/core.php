<?php
		...
		class wp_automatic {
			...
			function fetch_links($keyword, &$camp) {
				echo "<br>so I should now get some links from clickbank for keyword:".$keyword  ;
					...
					if ($camp_type == 'SoundCloud') {
						...
						if(in_array('OPT_SC_COMMENT', $camp_opt)){
							...
							if(trim($wp_automatic_sc_client) != ''){
								...
								$api_url = "http://api.soundcloud.com/tracks/$item_id/comments?client_id=$wp_automatic_sc_client&limit=25";
									...
									foreach ($comments_json  as $new_comment){
										...
										if($new_comment->kind == 'comment'){
											...
											$data = array(
													...
													'comment_author_email' => '',
													'comment_author_url' => $new_comment->user->permalink_url,
				...
				foreach ( $keywords as $keyword ) {
					...
					if (trim ( $keyword ) != '') {
						...
						if (count ( $res ) > 0) {
							...
							if ( stristr($camp_post_content, 'vid_views') || stristr($camp_post_content, 'vid_rating') || stristr($camp_post_content, 'vid_likes') || stristr($camp_post_content, 'vid_dislikes') || PARENT_THEME =='truemag' || PARENT_THEME =='newstube' ){
			...
			function gtranslate($title, $content, $from, $to) {
			   ...
			   if($to == 'nor') $to = 'no';
				...
				$sentenses_plain = preg_replace('{,,,\d*?]}', ']', $sentenses_plain);
