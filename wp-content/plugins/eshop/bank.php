...
switch ($eshopaction) {
    case 'redirect':
		...
		$subject .=__(": ",'eshop').$ebank->ipn_data['RefNr'];
		...
		$ebank->ipn_data['payer_email']=$array['ename'].' '.$array['eemail'].' ';
		 $body =  __("选择西联付款订单（线下付款方式）",'eshop')."\n";
		 $body .= "\n".__("来自：",'eshop').$ebank->ipn_data['payer_email'].__("；订单时间：",'eshop').date('m/d/Y');
		 $body .= __(" at ",'eshop').date('g:i A')."\n\n";
		 $body .= __('详情访问后台：','eshop').":\n";
		 if(isset($array['dbid']))
