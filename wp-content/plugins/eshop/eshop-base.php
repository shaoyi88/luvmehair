...
if(!isset($_GET['change'])){
	?>
	<div class="wrap">
	<div id="eshopicon" class="icon32"></div><h2><?php _e('谷歌Base设置','eshop'); ?></h2>
	...
	<li><?php echo sprintf( __('Google shopping 直接提交XML地址: <a href="%1$s">%1$s</a>   (注：不懂什么是Google shopping? 请百度：Google shopping)','eshop'), trim(site_url(),'/').'/?eshopbasefeed' ); ?></li>
	</ul>
	<?php	
	...
	if($max>0){
		...
		echo '<ul style="display:none;" id="eshopsubmenu" class="stuffbox">';
	...
	?>	
		...
		<?php
		//paginate
		echo '<div style="display:none;" class="paginate tablenav-pages stuffbox">';
		if($records!=$max){
			$eecho = $page_links;
		}
		if(isset($_GET['eshopall']))
...
//form checks:
	...
	if(is_numeric($change)){
		if(isset($_POST['submit'])){
			...
			if($err!=''){
				...
				echo'<div id="message" class="updated fade"><p>'.__('GD-Shop Base details for this product have been updated.','eshop').'</p></div>'."\n";
			}
		}
	...
	?>
		<div class="wrap">
		<div id="eshopicon" class="icon32"></div><h2><?php _e('GD-Shop Base Product','eshop'); ?></h2>
		<?php eshop_admin_mode(); ?>
