...
class eshop_metabox_class {
	...
	function on_admin_menu() {
		...
		$this->pagehook = add_options_page(__('外贸商城运营系统 - GD-Shop','eshop'), __('GD-Shop','eshop'), 'eShop_admin', ESHOP_SETTINGS_PAGE_NAME, array(&$this, 'on_show_page'));
		//register  callback gets call prior your own page gets rendered
		add_action('load-'.$this->pagehook, array(&$this, 'on_load_page'));
		add_action('load-'.$this->pagehook, array(&$this, 'on_load_page_scripts'));
		$help='
		<p><strong>' . __('GD-Shop help:') . '</strong></p>
		<ul>
		<li>需要帮助，请联系客服企业QQ: 800025660</li>
		</ul>';
		if($this->pagehook != ''){
	...
	//	add_meta_box('howto-metaboxes-sidebox-1', 'Sidebox 1 Title', array(&$this, 'on_sidebox_1_content'), $this->pagehook, 'side', 'core');
		...
		add_meta_box('eshop-admin', __('GD-Shop Admin','eshop'), array(&$this, 'on_contentbox_1_content'), $this->pagehook, 'normal', 'core');
	...
	function on_show_page() {
		...
		echo '<div id="eshopicon" class="icon32"></div><h2>'.__('外贸商城运营系统 - GD-Shop','eshop').'</h2>'."\n";
		//info:
		eshop_check_error();
		...
		if(isset($_GET['resetbase']) && $_GET['resetbase']=='yes'){
			...
			echo '<div id="message" class="updated fade"><p>'.__('GD-Shop谷歌Shopping已更新.','eshop').'</p></div>'."\n";
		}elseif(isset($_POST['submit'])){
			echo'<div id="message" class="updated fade"><p>'.__('外贸商城运营系统 - GD-Shop have been updated.','eshop').'</p></div>'."\n";
		}
		?>
		...
		<input type='hidden' name='option_page' value='eshop_settings' />
			...
			<div style="background:#EFEFEF;" id="poststuff" class="metabox-holder<?php echo 2 == $screen_layout_columns ? ' has-right-sidebar' : ''; ?>">
				<div id="side-info-column" style="background:#EFEFEF;" class="inner-sidebar">

					<?php do_meta_boxes($this->pagehook, 'side', $data); ?>
				</div>
				<div style="background:#EFEFEF;" id="post-body"<?php echo 2 == $screen_layout_columns ? ' class="has-sidebar"' : ''; ?> >
	...
	function on_contentbox_1_content($eshopoptions) {
		?>
		<fieldset>
		<label for="eshop_status"><?php _e('GD-Shop status','eshop'); ?></label>
