...
if (!function_exists('display_cart')) {
	function display_cart($shopcart, $change, $eshopcheckout, $pzone = '', $shiparray = '',$espost) {
		// The cart display.
		...
		if ($check > 0) {
			...
			$echo .= '<div class="eshopcart">   <h2 class="eshopcart-title">Shopping Cart</h2><table class="eshop cart">
			
			<thead>
			<tr class="thead">';

			$echo .= '<th id="cartItem' . $iswidget . '" class="nb" colspan="2">' . __('Item Description', 'eshop') . '</th>
			...
			foreach ($eshopcartarray as $productid => $opt) {
				...
				if (is_array($opt)) {
					...
					if ($opt["diy_option"]) {
						...
						$diy_option_arr = explode(',', substr($opt["diy_option"], 0, -1));
						$diy_option_str = '';
						...
						foreach($diy_option_arr as $v) {
							...
							$diy_option_str .= '<span class="eshopoptset"><span class="eshopoptname">' . $diy_option_name . ': </span>' . $diy_option_v[$diy_option_name]['title'] . ' </span><br />';
						} 
					}else{
						$diy_option_str = '';
					}

					$textdesc = '<a href="' . get_permalink($opt['postid']) . '">' . stripslashes($opt["pname"]) . ' <span class="eshopidetails">(ID: ' . $opt['postid'] . ')</span></a>' . $echooptset . '<br />' . $diy_option_str;
					$echoimg = $eimg;
					...
					$echo .= '<td><a target="_bank" class="itemref" href="' . get_permalink($opt['postid']) . '" title="view product">' . get_the_post_thumbnail($opt['postid'], array(100, 100)) . '</a></td>';
					...
					// $addoprice = $diy_option_price_totle + $addoprice;
					$opt["price"] += $addoprice;
					
					if (isset($_SESSION['eshop_discount' . $blog_id]) && eshop_discount_codes_check()) {
						$dtype=$_SESSION['eshop_discount_dtype'.$blog_id];
						if($dtype==1){
								$discount = is_discountable(calculate_total(),$eshopoptions['discount_type']) / 100;
								$disc_line = round($opt["price"] - ($opt["price"] * $discount), 2);
							}else{
								$discount = is_discountable(calculate_total(),$eshopoptions['discount_type']) ;
								$disc_line = round($opt["price"] - $discount, 2);
							}		
					}else{
						if (is_discountable(calculate_total(),$eshopoptions['discount_type']) > 0) {						
					...
					//批发价格2015-07-31
					$eshop_whs_data=maybe_unserialize(get_post_meta( $opt['postid'], '_eshop_whs',true ));
					
					if(is_array($eshop_whs_data)){
						$k=0;						
						foreach($eshop_whs_data as $val){
							if($k==0){
								if(($opt["qty"]>$val['qty_start']||$val['qty_start'] ==$opt["qty"])&&($opt["qty"] < $val['qty_end']||$opt["qty"] == $val['qty_end'])){
								    $opt["price"]=$opt["old_price"] + $val['price'];
							    }
							}else{
								if(($opt["qty"]>$val['qty_start'])&&($opt["qty"] < $val['qty_end']||$opt["qty"] == $val['qty_end'])){
								    $opt["price"]=$opt["old_price"] + $val['price'];
							    }
							}
							$k++;							
						}
						$opt["old_price"] = $opt["price"];
					}
					// end 批发价格 2015-07-31	

					//会员组优惠 2015-08-15
					
					$eshop_rolesprcie_data=maybe_unserialize(get_post_meta( $opt['postid'], '_eshop_rolesprcie',true ));
					$val=array();
					if(is_array($eshop_rolesprcie_data)){
						global $current_user;
						$current_user_roles = $current_user->roles[0]; 
						foreach($eshop_rolesprcie_data as $val){
							$rolesprcie[$val['user_roles']]=$val['price'];
						}
						if($rolesprcie[$current_user_roles]){
								$opt["price"]=$opt["old_price"] + intval($rolesprcie[$current_user_roles]);
						}						
					}
					// end 会员组优惠 2015-08-15
					if(!$_SESSION['eshopcart' . $blog_id][$productid]['new_price']){
						$_SESSION['eshopcart' . $blog_id][$productid]['new_price'] = $opt["price"];
					}
					...
					$eline = get_currency_price($eline);
					...
					$echo .= "</td>\n<td headers=\"cartTotal$iswidget prod" . $calt . $iswidget . "\" class=\"amts\" id='amts_" . $accid . $iswidget . "'>" . sprintf(__('%1$s%2$s', 'eshop'), $currsymbol, number_format_i18n($eline, __('2', 'eshop'))) . "</td>\n"; 
					// TAX
					if (isset($eshopoptions['etax']))
						$etax = $eshopoptions['etax'];
					if (($pzone != '' && isset($eshopoptions['tax']) && $eshopoptions['tax'] == '1') || ('yes' == $eshopoptions['downloads_only'] && isset($etax['unknown']) && $etax['unknown'] != '')) {
			...
			if($eshop_plugin_settings[$espost[eshop_payment]]['payfee']!=''){
                 $sub_total=$sub_total+$sub_total*($eshop_plugin_settings[$espost[eshop_payment]]['payfee']/100);
			}
			//2015-08-25 支付方式手续费
			$disc_applied = '';
			if (is_discountable(calculate_total()) > 0) {
					...
					if($dtype==1){
						...
						$discount = is_discountable(calculate_total());
				        $disc_applied = '<small>(' . __('Including Discount of <span>$'.number_format_i18n(round($discount, 2), 2).'</span>', 'eshop'). ')</small>';
					}
			...
			$echo .= "<tr class=\"stotal\"><td headers=\"subtotal$iswidget cartTotal$iswidget\" class=\"amts lb\" colspan=\"4\"><b>" . __('Sub-Total', 'eshop') . ' ' . $disc_applied . "</b>: " . sprintf(__('%1$s%2$s', 'eshop'), $currsymbol, number_format_i18n($sub_total, __('2', 'eshop'))) . "</td>$emptycell</tr>\n";
			...
			if ($pzone != '' || ('yes' == $eshopoptions['downloads_only'] && isset($etax['unknown']) && $etax['unknown'] != '')) {
				if ($pzone != '') {
					...
					$echo .= 'Shipping Cost:</th>
					<td headers="cartItem scharge" class="amts lb" colspan="3">' . sprintf(__('%1$s%2$s', 'eshop'), $currsymbol, number_format_i18n($shipping, __('2', 'eshop'))) . '</td>';
				...
				$echo .= '<tr class="total"><th id="cTotal' . $iswidget . '" class="leftb">' . __('Total Order Charges: ', 'eshop') . "</th>\n<td headers=\"cTotal$iswidget cartTotal$iswidget\"  colspan=\"3\" class = \"totalall amts lb\"><strong>" . sprintf(__('%1$s%2$s <span>%3$s</span>', 'eshop'), $currsymbol, number_format_i18n($final_price, __('2', 'eshop')), $excltax) . "</strong></td>";
...
if (!function_exists('is_discountable')) {
	function is_discountable($total,$t) {
		...
		for ($x = 1;$x <= 3;$x++) {
			if ($eshopoptions['discount_spend' . $x] != '')
			    ...
			    $edisc_money[$eshopoptions['discount_spend' . $x]] = $eshopoptions['discount_money_value' . $x];
		} 
		if (isset($edisc) && is_array($edisc) &&$t==1) {
			krsort($edisc);
		...
		if (isset($edisc_money) && is_array($edisc_money) &&$t==2) {
			krsort($edisc_money);
			foreach ($edisc_money as $amt => $percent) {
				if ($amt <= $total)
					return $percent;
			} 
			$percent = 0;
...
if (!function_exists('valid_eshop_discount_code')) {
	function valid_eshop_discount_code($code) {
		...
		if (is_object($row)) {
			$_SESSION['eshop_discount_dtype'.$blog_id]=$row -> dtype;
			switch ($row -> dtype) {
				case '1':
					if ($row -> remain == '' || $row -> remain > 0)
						return true;
...
if (!function_exists('eshopCartFields')) {
	function eshopCartFields() {
		...
		foreach ($eshopcartarray as $productid => $opt) {
			...
			$echo .= "\n  <input type=\"hidden\" name=\"diy_option_" . $x . "\" value=\"" . substr($opt["diy_option"], 0, -1) . "\" />";
			/**
			 * options
			 */
			$addoprice = 0;
	...
	function orderhandle($espost, $checkid) {
		...
		if ($eshopching != $checkid) {
			...
			while ($i <= $espost['numberofproducts']) {
				...
				$diy_option = esc_sql($espost['diy_option_' . $i]);
				...
				if ($dlchk != '') {
					...
					(checkid, item_id,item_qty,item_amt,tax_rate,tax_amt,optname,post_id,option_id,down_id,optsets,weight,diy_option)values(
					'$checkid','$item_id','$item_qty','$item_amt', '$tax_rate', '$tax_amt', 
					'$optname','$post_id','$option_id',
					'$dlchk','$optset','$weight','$diy_option');");
					...
					$queryitem = $wpdb -> query("INSERT INTO $itemstable
					(checkid, item_id,item_qty,item_amt,tax_rate,tax_amt,optname,post_id,option_id,optsets,weight,diy_option)values(
					'$checkid','$item_id','$item_qty','$item_amt','$tax_rate', '$tax_amt', 
					'$optname','$post_id','$option_id','$optset','$weight','$diy_option');");
				} 
				$i++;
			} 
...
if (!function_exists('create_eshop_error')) {
	...
	function create_eshop_error($error) {
		...
		?>
		<div class="error fade"><?php echo $error;
...
if (!function_exists('eshop_error_message')) {
	function eshop_error_message($num) {
		$messages = array('1' => __('产品参数未填写完整，请检查！', 'eshop'),
			'2' => __('价格填写不规范，请填写数字', 'eshop'),
			'3' => __('重量填写不规范，请填写数字', 'eshop'),
			'4' => __('库存填写不规范，请填写数字', 'eshop'),
			'5' => __('价格填写不规范，请填写数字', 'eshop'),
			'100' => __('设置已更新.', 'eshop')
			);
		...
		if ($num < 100 && array_key_exists($num, $messages)) {
			...
			?>
		<div class="error fade"><p><?php echo $messages[$num];
		...
		<?php
			...
			?>
			<div id="message" class="updated fade"><p><?php echo $messages[$num];
...
if (!function_exists('eshop_cart_process')) {
	function eshop_cart_process($data = '') {
		...
		if (!isset($_POST['save'])) {
			...
			// diy_option 2015-02-12 add
			$option_price = 0;
			foreach($_POST['diy_option'] as $key => $val) {
				$option_arr = explode('@', $val);
				$value = $key . ':' . $val . ",";
				$s_diy_option_str .= $value;
				$option_price += $option_arr[1];
			} 
			$iprice = $iprice + $option_price; 
			// end 2015-02-12 add
			$identifier = $pid . $option . $postid . $optset . $s_diy_option_str; 
			// $needle=array(" ","-","$","\r","\r\n","\n","\\","&","#",";");
			...
			if (isset($_SESSION['eshopcart' . $blog_id][$identifier])) {
				...
				//批发价格2015-07-31
				$_SESSION['eshopcart' . $blog_id][$identifier]['old_price'] = $iprice;
				
				$eshop_whs_data=maybe_unserialize(get_post_meta( $postid, '_eshop_whs',true ));
				
				if(is_array($eshop_whs_data)){
					$k=0;
						foreach($eshop_whs_data as $val){
							if($k==0){
								if(($qty>$val['qty_start']||$val['qty_start'] ==$qty)&&($qty < $val['qty_end']||$qty == $val['qty_end'])){
								    $iprice=$iprice + $val['price'];
							    }
							}else{
								if(($qty>$val['qty_start'])&&($qty < $val['qty_end']|| $qty== $val['qty_end'])){
								    $iprice=$iprice + $val['price'];
							    }
							}
							$k++;							
				...
				//会员组优惠 2015-08-15
					...
					$eshop_rolesprcie_data=maybe_unserialize(get_post_meta( $postid, '_eshop_rolesprcie',true ));
					$val=array();
					if(is_array($eshop_rolesprcie_data)){
						global $current_user;
						$current_user_roles = $current_user->roles[0]; 
						...
						if($rolesprcie[$current_user_roles]){
							$iprice=$iprice + intval($rolesprcie[$current_user_roles]);
						}						
					}
					// end 会员组优惠 2015-08-15
				...
				$_SESSION['eshopcart' . $blog_id][$identifier]['diy_option'] = $s_diy_option_str;
			...
			foreach($orowres as $orow) {
				if (isset($newoptings[$x]['id']) && $orow -> id == $newoptings[$x]['id']) {
					if ((isset($newoptings[$x]['type']) && isset($newoptings[$x]['text']) && trim($newoptings[$x]['text']) != '' && ($newoptings[$x]['type'] == '2' || $newoptings[$x]['type'] == '3'))) {
						$oset[] = '<span class="eshopoptset"><span class="eshopoptname">' . stripslashes($orow -> name) . ": </span>" . '' . stripslashes($newoptings[$x]['text']) . '</span>';
					} elseif (($orow -> type == '2' || $orow -> type == '3') && !isset($newoptings[$x]['text']))
						$xxxx = '';
					else
						$oset[] = '<span class="eshopoptset"><span class="eshopoptname">' . stripslashes($orow -> oname) . ": </span>" . '' . stripslashes($orow -> name) . '</span>';
