   ...
   case 'paypalipn':          // Paypal is calling page for IPN validation...
		...
		*/
			...
			if($eshopoptions['status']=='live'){
				...
				$subject = __('新订单提醒：Paypal付款 -','eshop');
			}else{
			...
			if($p->ipn_data['receiver_email']!= $eshopoptions['business']){
				$astatus='Failed';
				$txn_id .= __(" - Fraud",'eshop');
				$extradetails .= __("The business email address in GD-Shop does not match your main email address at Paypal.",'eshop');
			}
			...
			if($astatus=='Pending' && $p->ipn_data['payment_status']=='Completed'){
				...
				$subject .=__("未付款订单",'eshop');
				$ok='no';
				$extradetails .= __("You have received a refund notification, GD-Shop doesn't know how to handle these, but details of the notification are included below.",'eshop');
			}else{
				$query2=$wpdb->query("UPDATE $detailstable set status='Failed',transid='$txn_id' where checkid='$checked'");
				$subject .=__("交易失败订单",'eshop');
				$ok='no';
				$extradetails .= __("The transaction was not completed successfully. GD-Shop could not validate the order. Please double check all settings in GD-Shop and at paypal to confirm they are both correct.",'eshop');
			...
			// email to business a complete copy of the notification from paypal to keep!!!!!
			 $body =  __("Paypal新订单（在线支付）",'eshop')."\n";
			 $body .= "\n".__("来自：",'eshop').$p->ipn_data['payer_email'].__("；订单时间：",'eshop').date('m/d/Y');
