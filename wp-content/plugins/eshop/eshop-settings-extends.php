...
function admin_notice_zpXnvPSiCDpL() {
  ...
  if ( ! get_user_meta($user_id, 'ignore_notice') && is_admin()) {
        //echo '<br /><div class="updated"><p>'; 
          //echo'此域名没有被授权或未续费，系统功能不能正常使用，请联系 <a href="http://www.gd-shop.cn/" target="_blank">谷道科技</a> 进行域名授权或续费！'; 
          //echo "</p></div>";
  }
}
...
if ( $KEY_VALUE_MD != get_transient( $PRODUCT_ID.'_license_'.$KEY_NAME_MD)){		
		if(!is_admin()&& !is_login_page()){
			wp_die('<p style="text-align:center;">此域名没有被授权，功能不能正常使用，请正版用户联系<a href="http://www.gd-shop.cn/" target="_blank">谷道科技</a>进行域名授权。</p>
    			<p style="text-align:center;"><a href="javascript:window.location.reload();">刷新</a>&nbsp;&nbsp;<a href="/eo-admin/">进入后台</a></p>');
		}
		else {
...
class eshop_setting_downloads_class extends eshop_metabox_class {
	function on_load_page() {
		add_meta_box('eshop-downloads', __('GD-Shop 下载设置','eshop'), array(&$this, 'downloads_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('eshop-downloads-only', __('GD-Shop 仅支持产品下载模式','eshop'), array(&$this, 'downloads_box_2'), $this->pagehook, 'normal', 'core');
		add_meta_box('eshop-downloads-emails', __('邮箱设置','eshop'), array(&$this, 'downloads_box_3'), $this->pagehook, 'normal', 'core');
		add_filter('eshop_error_messages',array(&$this, 'eclass_errors'));
		add_screen_option('layout_columns', array('max' => 2, 'default' => 2) );
	}
	function downloads_box($eshopoptions) {
		?>
		<fieldset><br />
			<label for="eshop_downloads_num"><?php _e('下载次数','eshop'); ?></label><input id="eshop_downloads_num" name="eshop_downloads_num" type="text" value="<?php echo $eshopoptions['downloads_num']; ?>" size="5" /><br />
		</fieldset>
		<fieldset><br />
			<label for="eshop_downloads_hideall"><?php _e('隐藏下载模式信息 (非下载产品，如软件、CDkey之类的常规产品，请选择Yes，如果选择No，购物时不会要求客户填写收货地址，只需填写邮箱！)','eshop'); ?></label>
	...
	function downloads_box_2($eshopoptions) {
		?>
		<fieldset><br />
			<label for="eshop_downloads_only"><?php _e('仅支持下载模式 (非下载产品，如软件、CDkey之类的常规产品，请选择No！)','eshop'); ?></label>
	...
	function downloads_box_3($eshopoptions) {
		?>
		<fieldset><br />
			<label for="eshop_downloads_email"><?php _e('自动转发邮件中是否显示 产品下载信息(注：如果是西联汇款，完成交易但未线下转款的也会自动发，如果启用线下汇款，请关闭此功能)','eshop'); ?></label>
...
class eshop_setting_discount_class extends eshop_metabox_class {
	function on_load_page() {
		add_meta_box('eshop-discounts', __('GD-Shop 折扣设置','eshop'), array(&$this, 'eadmin_box'), $this->pagehook, 'normal', 'core');
		add_filter('eshop_error_messages',array(&$this, 'eclass_errors'));
		add_screen_option('layout_columns', array('max' => 2, 'default' => 2) );
	}
	...
	function eadmin_box($eshopoptions) {
		...
		<fieldset>
			...
			<thead>
				<tr>
				<th id="elevel"><?php _e('折扣设置','eshop'); ?></th>
				<th id="espend"><?php _e('购物车总金额 ≥','eshop'); ?></th>
				<th id="ediscount"><input name="eshop_discount_type" type="radio" value="1" <?php if  ($eshopoptions['discount_type']==1) echo "checked";?>/><?php _e('% 百分比折扣 ','eshop'); ?></th>
				<th id="ediscount"><input name="eshop_discount_type" type="radio" value="2" <?php if  ($eshopoptions['discount_type']==2) echo "checked";?> /><?php _e('金额 ','eshop'); ?></th>
				</tr>
			</thead>
			<tbody>
				<?php
				...
				<td headers="elevel ediscount row<?php echo $x ?>"><label for="eshop_discount_money_value<?php echo $x ?>"><?php _e('Discount','eshop'); ?></label>$<input id="eshop_discount_money_value<?php echo $x ?>" name="eshop_discount_money_value<?php echo $x ?>" type="text" value="<?php echo $eshopoptions['discount_money_value'.$x]; ?>" size="5" maxlength="5" /></td>
			...
			<p><b>超过该金额包邮：</b>$<input id="eshop_discount_shipping" name="eshop_discount_shipping" type="text" value="<?php echo $eshopoptions['discount_shipping']; ?>" size="5" /></p>
	</fieldset>
	<?php
	}
	function on_save_changes() {
		...
		$eshopoptions['discount_type']=esc_sql($_POST['eshop_discount_type']);
		...
		for ($x=1;$x<=3;$x++){
			if(is_numeric($_POST['eshop_discount_spend'.$x]) && is_numeric($_POST['eshop_discount_value'.$x])){
				...
				$eshopoptions['discount_money_value'.$x]=esc_sql($_POST['eshop_discount_money_value'.$x]);
...
class eshop_setting_general_class extends eshop_metabox_class {
	function on_load_page() {
		...
		add_meta_box('orders-display', __('订单显示','eshop'), array(&$this, 'orders_box'), $this->pagehook, 'side', 'core');
		add_meta_box('business-details', __('商务信息','eshop'), array(&$this, 'business_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('product-options', __('产品选项','eshop'), array(&$this, 'product_options_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('product-details', __('商务信息','eshop'), array(&$this, 'product_details_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('display-options', __('显示选项','eshop'), array(&$this, 'display_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('checkout-options', __('支付选项','eshop'), array(&$this, 'checkout_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('eshop-cacheability', __('缓存设置','eshop'), array(&$this, 'cache_box'), $this->pagehook, 'side', 'low');
		add_filter('eshop_error_messages',array(&$this, 'eclass_errors'));
		add_screen_option('layout_columns', array('max' => 2, 'default' => 2) );
	}
	...
	function business_box ($eshopoptions){
		?>
		<fieldset><br />
		<label for="eshop_from_email"><?php _e('商城主邮箱 <small>(如果选择SMTP转发，网站主邮箱、商城主邮箱、SMTP邮箱 请填写同一邮箱地址！)</small>','eshop'); ?></label><input id="eshop_from_email" name="eshop_from_email" type="text" value="<?php echo $eshopoptions['from_email']; ?>" size="30" /><br />
		<label for="eshop_sysemails"><?php _e('备用邮箱 <small>(给客户发邮件通知时可以选择以下邮箱)</small>','eshop'); ?></label>
	...
	function product_options_box ($eshopoptions){
		?>
		<fieldset><br /><div style="display:none;">
		<label for="eshop_options_num"><?php _e('产品基础信息数量 <small>(默认为1条，勿修改)</small>','eshop'); ?></label><input id="eshop_options_num" name="eshop_options_num" type="text" value="<?php echo $eshopoptions['options_num']; ?>" size="5" /><br /> </div>
		<label for="eshop_stock_control"><?php _e('库存功能','eshop')._e(' <small>(打开该功能后必须填写产品的库存数量，否则库存为0，不显示购物车)</small>','eshop'); ?></label>
			...
			?><div>
			<label for="eshop_min_qty"><?php _e('最小购买数量限制 <small>(留空则无限制)</small>','eshop'); ?></label><input id="eshop_min_qty" name="eshop_min_qty" type="text" value="<?php echo $eshopoptions['min_qty']; ?>" size="5" /><br />
			<label for="eshop_max_qty"><?php _e('最大购买数量限制 <small>(留空则无限制)</small>','eshop'); ?></label><input id="eshop_max_qty" name="eshop_max_qty" type="text" value="<?php echo $eshopoptions['max_qty']; ?>" size="5" /><br /></div>
			<?php if(!isset($eshopoptions['sale_prices'])) $eshopoptions['sale_prices']=0; ?>
			<label for="eshop_sale_prices"><?php _e('同时使用原价和卖价 <small>(模版网站默认为Yes，模版均使用 原价和卖价，同时根据这2个价格自动进行折扣计算，如果某产品不需要显示原价，请在产品编辑页修改)</small>)','eshop'); ?></label>
	...
	function product_details_box ($eshopoptions){
		...
		<fieldset><br />
			...
			if(!isset($eshopoptions['details']['display']))
				$eshopoptions['details']['display']='';
			?><br />
			<label for="eshop_details_display"><?php _e('每个产品页面均显示产品参数. <small>(默认选择Yes，否则不显示产品参数，例如颜色、尺寸)</small>','eshop'); ?></label>
			...
			<div style="display:none;"><label for="eshop_cart_nostock"><?php _e('无库存提示','eshop'); ?></label><input id="eshop_cart_nostock" name="eshop_cart_nostock" type="text" value="<?php echo $eshopoptions['cart_nostock']; ?>" size="30" /><br /></div>
			
			<label for="eshop_currency_symbol"><?php _e('主币种  <small>(支持多币种的模版不能在这里修改)</small>','eshop'); ?>
</label><input id="eshop_currency_symbol" name="eshop_currency_symbol" type="text" value="$" size="10" /><br />
			<label for="eshop_addtocart_image"><?php _e('Add Cart按钮使用图片或按钮？ <small>(所有模版默认使用Button)</small>','eshop'); ?></label>
			...
			<p><label for="eshop_ajax_cart"><?php _e('添加购物车之后默认跳转到购物车页面 <small>(所有模版默认使用Yes，定制方案可修改)</small>','eshop'); ?></label>
			...
			?>
			<p><strong><?php _e('在购物按钮前后加上备注 - 支持 HTML','eshop'); ?></strong></p>
			<label for="eshop_cart_text"><?php _e('文本：','eshop'); ?></label><input id="eshop_cart_text" name="eshop_cart_text" type="text" value="<?php echo stripslashes(esc_attr($eshopoptions['cart_text'])); ?>" size="60" /><br />
			<label for="eshop_cart_text_where"><?php _e('位置：','eshop'); ?></label>
	...
	function display_box ($eshopoptions){
		?>
		<fieldset><br />
			<label for="eshop_show_forms"><?php _e('产品列表页显示购物车按钮 <small>(模版均支持)</small>','eshop'); ?></label>
			...
			<label for="eshop_fold_menu"><?php _e('隐藏导航菜单的子菜单','eshop'); ?></label>
			...
			<label for="eshop_hide_cartco"><?php _e('该产品被添加进购物车之后，不再显示该产品的Add Cart按钮','eshop'); ?></label>
			...
			<label for="eshop_search_img"><?php _e('产品搜索结果中显示产品图片 <small>(需模版支持)</small>','eshop'); ?></label>
			...
			<label for="eshop_credits"><?php _e('支付时显示收款信用卡具体信息 <small>(建议默认选择No)</small>','eshop'); ?></label>
	...
	function checkout_box ($eshopoptions){
		?>
		<fieldset><br />
		<label for="eshop_hide_addinfo"><?php _e('支付时 隐藏 备注信息 Additional information.','eshop'); ?></label>
		...
		<label for="eshop_hide_shipping"><?php _e('隐藏收货地址 <small>(下载类商城模版支持)</small>','eshop'); ?></label>
		...
		<label for="eshop_downloads_only"><?php _e('简洁支付流程 <small>(部分模版支持)</small>','eshop'); ?></label>
		<select name="eshop_downloads_only" id="eshop_downloads_only">
		<?php
		if('yes' == $eshopoptions['downloads_only']){
			echo '<option value="no" selected="selected">'.__('Yes','eshop').'</option>';
		}else{
			echo '<option value="no" selected="selected">'.__('Yes','eshop').'</option>';
		}
		?>
		</select><br />
		<label for="eshop_tandc_use"><?php _e('支付时显示：必选按钮 ((如：购买政策！)).','eshop'); ?></label>
		...
		<label for="eshop_tandc"><?php _e('必选按钮提示； <small>(例如：同意本站购买政策！)</small>','eshop'); ?></label><input id="eshop_tandc" name="eshop_tandc" type="text" value="<?php echo $eshopoptions['tandc']; ?>" size="60" /><br />
		<label for="eshop_tandc_id"><?php _e('页面ID (连接到政策页面，详细操作参考教程).','eshop'); ?></label><input id="eshop_tandc_id" name="eshop_tandc_id" type="text" value="<?php echo $eshopoptions['tandc_id']; ?>" size="6" /><br />
		<?php if (eshop_wp_version('3')){ ?>
		<label for="eshop_users"><?php _e('未注册直接购买','eshop'); ?></label>
		...
		<label for="eshop_zero"><?php _e('允许0金额订单.','eshop'); ?></label>
	...
	function cache_box ($eshopoptions){
		?>
		<fieldset><br />
		<label for="eshop_set_cacheability">停用缓存页面 <small>(购物流程页面不要使用缓存，否则会引起支付失效)</small></label>
	...
	function eadmin_box($eshopoptions) {
		...
		<br /><br />
			<label for="eshop_status"><?php _e('商城状态','eshop'); ?></label>
			<select name="eshop_status" id="eshop_status">
			<?php
			if('live' == $eshopoptions['status']){
				echo '<option value="live" selected="selected">'.__('运行','eshop').'</option>';
				echo '<option value="testing">'.__('测试','eshop').'</option>';
			}else{
				echo '<option value="live">'.__('运行','eshop').'</option>';
				echo '<option value="testing" selected="selected">'.__('测试','eshop').'</option>';
			}
			?>
			</select><br />
			<label for="eshop_sale"><?php _e('销售','eshop'); ?></label>
			<select name="eshop_sale" id="eshop_sale">
			<?php
			if(isset($eshopoptions['sale']) && 'yes' == $eshopoptions['sale']){
				echo '<option value="yes" selected="selected">'.__('销售','eshop').'</option>';
				echo '<option value="no">'.__('停止销售','eshop').'</option>';
			}else{
				echo '<option value="yes">'.__('销售','eshop').'</option>';
				echo '<option value="no" selected="selected">'.__('停止销售','eshop').'</option>';
			}
			?>
			</select><br />
		</fieldset>
	...
	function orders_box($eshopoptions) {
			?>
			<fieldset><br />
				<label for="eshop_records"><?php _e('每页显示订单数量','eshop'); ?></label><input id="eshop_records" name="eshop_records" type="text" value="<?php echo $eshopoptions['records']; ?>" size="5" /><br />
			</fieldset>
		<?php
	}
	function on_show_page() {
			...
			add_meta_box('eshop-admin', __('商城运行状态','eshop'), array(&$this, 'eadmin_box'), $this->pagehook, 'side', 'high');
			...
			echo '<div id="eshopicon" class="icon32"></div><h2>GD-Shop 基础设置</h2>'."\n";
			//info:
			eshop_check_error();
			eshop_admin_mode();
			//the submenu 
			...
			<input type='hidden' name='option_page' value='eshop_settings' />
				...
				<div style="background:#EFEFEF;" id="poststuff" class="metabox-holder<?php echo 2 == $screen_layout_columns ? ' has-right-sidebar' : ''; ?>">
					<div style="background:#EFEFEF;" id="side-info-column" class="inner-sidebar">
						<p class="submit">
							<input type="submit" name="submit" class="button-primary" value="<?php _e('保存更新','eshop') ?>" />
						</p>
						<?php do_meta_boxes($this->pagehook, 'side', $data); ?>
					</div>
					<div style="background:#EFEFEF;" id="post-body" class="has-sidebar">
						<div style="background:#EFEFEF;" id="post-body-content" class="has-sidebar-content">
...
class eshop_setting_merchant extends eshop_metabox_class {
	function on_load_page() {
		add_meta_box('eshop-m-general', __('基本设置','eshop'), array(&$this, 'general_box'), $this->pagehook, 'side', 'core');
		add_meta_box('eshop-m-paypal', __('Paypal','eshop'), array(&$this, 'paypal_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('eshop-m-amazon', __('Amazon','eshop'), array(&$this, 'amazon_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('eshop-m-dhpay', __('DHPay(敦煌支付)信用卡通道','eshop'), array(&$this, 'dhpay_box'), $this->pagehook, 'normal', 'core');

		add_meta_box('eshop-m-alipay', __('支付宝','eshop'), array(&$this, 'alipay_box'), $this->pagehook, 'normal', 'core');
		...
		add_meta_box('eshop-m-cash', __('银行汇款(线下支付)','eshop'), array(&$this, 'cash_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('eshop-m-bank', __('西联汇款(线下支付)','eshop'), array(&$this, 'bank_box'), $this->pagehook, 'normal', 'core');
	...
	function general_box($eshopoptions) {
		...
		<br />
	<label for="eshop_location"><?php _e('商城运营所在国家','eshop'); ?></label>
			<select style="width:183px;" name="eshop_location" id="eshop_location">
			<?php
			...
			<label for="eshop_currency"><?php _e('商城币种 (默认主语种为$usd,如果需要修改主语种，请联系谷道科技；)','eshop'); ?></label>
	...
	?>
		<fieldset>
			...
			?>
			<p class="cbox"><br /><input id="eshop_method" name="eshop_method[]" type="checkbox" value="paypal"<?php if(in_array('paypal',(array)$eshopoptions['method'])) echo ' checked="checked"'; ?> /><label for="eshop_method" class="eshopmethod"><?php _e('使用Paypal收款','eshop'); ?></label></p>
			<label for="eshop_business"><?php _e('Paypal主帐号 <small>(请填写Paypal主帐号邮箱，Paypal订单提醒邮件同时也会发送到该邮箱)</small>','eshop'); ?></label><input id="eshop_business" name="eshop_business" type="text" value="<?php echo $eshopoptions['business']; ?>" size="30" /><br />
			<!--<label for="eshop_business_sec"><?php _e('Paypal备用帐号','eshop'); ?></label><input id="eshop_business_sec" name="eshop_business_sec" type="text" value="<?php echo $eshopoptions['business_sec']; ?>" size="30" /><br />
			<label for="eshop_paypal_noemail"><?php _e('本站支付界面是否显示Paypal帐号 <small>(非Paypal支付显示，跳转到Paypal后会显示帐号或全称，请自己在Paypal设置)</small>','eshop'); ?></label>
			...
			<label for="eshop_business"><?php _e('购物车总价限额','eshop'); ?></label>$<input id="eshop_business" name="eshop_paylimit" type="text" value="<?php echo $eshopoptions['paylimit']; ?>" size="30" /><br />

			<label for="eshop_business"><?php _e('购物车总价手续费','eshop'); ?></label><input id="eshop_payfee" name="eshop_payfee" type="text" value="<?php echo $eshopoptions['payfee']; ?>" size="30" />%<br />
		</fieldset>
		<?php
	}
	function amazon_box($eshopoptions) {
	?>
		<fieldset>
			<?php 		
			$this->show_img('amazon');
			?>
			<p class="cbox"><br /><input id="eshop_method" name="eshop_method[]" type="checkbox" value="amazon"<?php if(in_array('amazon',(array)$eshopoptions['method'])) echo ' checked="checked"'; ?> /><label for="eshop_method" class="eshopmethod"><?php _e('使用Amazon收款','eshop'); ?></label></p>
		</fieldset>
		<?php
	}
	function dhpay_box($eshopoptions) {
	?>
		<fieldset>
			<?php 		
			$this->show_img('dhpay');
			if(isset($eshopoptions['dhpay']))
				$dhpay = $eshopoptions['dhpay']; 
			else
				$dhpay[merchant_id]=$dhpay[private_key]='';	
			?>
			<p class="cbox"><br /><input id="eshop_method" name="eshop_method[]" type="checkbox" value="dhpay"<?php if(in_array('dhpay',(array)$eshopoptions['method'])) echo ' checked="checked"'; ?> /><label for="eshop_method" class="eshopmethod">使用DHPay收款 - <a target="_blank" href="https://www.dhpay.com/re/reg.do?source=15012710054526">点击注册,0开户费</a></label></p>
			<label for="eshop_dhpay_merchant_id"><?php _e('Merchant id','eshop'); ?></label><input id="eshop_dhpay_merchant_id" name="dhpay[merchant_id]" type="text" value="<?php echo $dhpay[merchant_id]; ?>" size="30" /><br />
			<label for="eshop_dhpay_private_key"><?php _e('Private key','eshop'); ?></label><input id="eshop_dhpay_private_key" name="dhpay[private_key]" type="text" value="<?php echo $dhpay[private_key]; ?>" size="30" /><br />

			<label for="eshop_business"><?php _e('购物车总价限额','eshop'); ?></label>$<input id="eshop_business" name="dhpay[eshop_paylimit]" type="text" value="<?php echo $dhpay['paylimit']; ?>" size="30" /><br />

			<label for="eshop_business"><?php _e('购物车总价手续费','eshop'); ?></label><input id="eshop_payfee" name="dhpay[eshop_payfee]" type="text" value="<?php echo $dhpay['payfee']; ?>" size="30" />%<br />

			<p>DHpay是敦煌网旗下独立第三方支付工具，自2010年其服务于敦煌网在线贸易平台，截止至今已支持全球224个国家和地区400万规模的买家实现在线跨境支付，是国内率先实现跨境支付的机构。<br />
DHpay致力于让从事跨境B2C、B2B独立网站运营的个人和企业能够安全、便捷、快速地完成外币在线付款和收款。<br />
DHpay凭借其强大的风控和支付体系，成为您值得信赖的跨境支付专家，助您网络世界财富。</p>
		</fieldset>
		<?php
	}
	
	function alipay_box($eshopoptions) {
	?>
		<fieldset>
			<?php 		
			$this->show_img('alipay');
			if(isset($eshopoptions['alipay']))
				$alipay = $eshopoptions['alipay']; 
			else
				$alipay[merchant_id]=$alipay[private_key]='';	
			?>
			<p class="cbox"><br /><input id="eshop_method" name="eshop_method[]" type="checkbox" value="alipay"<?php if(in_array('alipay',(array)$eshopoptions['method'])) echo ' checked="checked"'; ?> /><label for="eshop_method" class="eshopmethod">使用alipay收款</label></p>
			<label for="eshop_alipay_partner"><?php _e('合作身份者id','eshop'); ?></label><input id="eshop_alipay_merchant_id" name="alipay[partner]" type="text" value="<?php echo $alipay[partner]; ?>" size="30" /><br />

			<label for="eshop_alipay_seller_email"><?php _e('收款支付宝账号','eshop'); ?></label><input id="eshop_alipay_seller_email" name="alipay[seller_email]" type="text" value="<?php echo $alipay[seller_email]; ?>" size="30" /><br />
			
			<label for="eshop_alipay_key"><?php _e('安全检验码','eshop'); ?></label><input id="eshop_alipay_private_key" name="alipay[key]" type="text" value="<?php echo $alipay['key']; ?>" size="30" /><br />
			<p>支付宝支付。</p>
		</fieldset>
		<?php
	}
	...
	function cash_box($eshopoptions) {
		...
		<fieldset><br />
			...
			$this->show_img('cash');
			?>
			<p><?php _e('<strong>注意:</strong> 银行汇款不能在线转账，客户选择银行汇款后，订单会自动交易完成，并提示银行信息！<br/>客户汇款后需要在My Order发送汇款信息，订单管理中可以看到选择银行汇款的客户，可以直接发Email和客户取得联系，帮助其汇款顺利完成！','eshop'); ?></p>
			<?php 
			...
			?>
			<p class="cbox"><input id="eshop_methode" name="eshop_method[]" type="checkbox" value="cash"<?php if(in_array('cash',(array)$eshopoptions['method'])) echo ' checked="checked"'; ?> /><label for="eshop_methode" class="eshopmethod"><?php _e('启用银行汇款','eshop'); ?></label></p>
			<label for="eshop_cashemail"><?php _e('订单提醒Email - 订单提醒邮件会发送到该邮箱','eshop'); ?></label><input id="eshop_cashemail" name="cash[email]" type="text" value="<?php echo $eshopcash['email']; ?>" size="30" maxlength="50" /><br />
			<label for="eshop_cashrename"><?php _e('银行全称','eshop'); ?></label><input id="eshop_cashrename" name="cash[rename]" type="text" value="<?php echo $eshopcash['rename']; ?>" size="30" maxlength="50" /><br />
		</fieldset>
	<?php
	}
	function bank_box($eshopoptions) {
		...
		<fieldset><br />
			...
			$this->show_img('bank');
			?>
			<p><?php _e('<strong>注意:</strong> 西联汇款不能在线转账，客户选择西联汇款后，订单会自动交易完成，并提示西联汇款信息！<br/>客户汇款后需要在My Order发送汇款信息，订单管理中可以看到选择西联汇款的客户，可以直接发Email和客户取得联系，帮助其汇款顺利完成！','eshop'); ?></p>
			<?php 
			...
			?>
			<p class="cbox"><input id="eshop_methodbk" name="eshop_method[]" type="checkbox" value="bank"<?php if(in_array('bank',(array)$eshopoptions['method'])) echo ' checked="checked"'; ?> /><label for="eshop_methodbk" class="eshopmethod"><?php _e('使用西联汇款','eshop'); ?></label></p>
			<label for="eshop_bankemail"><?php _e('订单提醒Email - 订单提醒邮件会发送到该邮箱','eshop'); ?></label><input id="eshop_bankemail" name="bank[email]" type="text" value="<?php echo $eshopbank['email']; ?>" size="30" maxlength="50" /><br />
			<label for="eshop_bankrename"><?php _e('西联外汇全称','eshop'); ?></label><input id="eshop_bankrename" name="bank[rename]" type="text" value="<?php echo $eshopbank['rename']; ?>" size="30" maxlength="50" /><br />
		</fieldset>	
		<?php
	}
	...
	function on_save_changes() {
		...
		$eshopoptions['business'] = $paypalpost['business'] = esc_sql(trim($_POST['eshop_business']));
		$eshopoptions['paylimit'] = $paypalpost['paylimit'] = esc_sql($_POST['eshop_paylimit']);
		$eshopoptions['payfee'] = $paypalpost['payfee'] = esc_sql($_POST['eshop_payfee']);
		$eshopoptions['paypal']=$paypalpost;
		//dhpay
		$dhpaypost['merchant_id']=esc_sql($_POST['dhpay']['merchant_id']);
		$dhpaypost['private_key']=esc_sql($_POST['dhpay']['private_key']);
		$dhpaypost['paylimit']=esc_sql($_POST['dhpay']['eshop_paylimit']);
		$dhpaypost['payfee']=esc_sql($_POST['dhpay']['eshop_payfee']);

		$eshopoptions['dhpay']=$dhpaypost;

		//alipay
		$alipaypost['partner']=esc_sql($_POST['alipay']['partner']);
		$alipaypost['seller_email']=esc_sql($_POST['alipay']['seller_email']);
		$alipaypost['key']=esc_sql($_POST['alipay']['key']);
		$eshopoptions['alipay']=$alipaypost;
...
class eshop_setting_tax extends eshop_metabox_class {
	function on_load_page() {
		add_meta_box('eshop-tax-action', __('启用进出口税','eshop'), array(&$this, 'taxenable_box'), $this->pagehook, 'side', 'core');
		add_meta_box('eshop-tax-setup', __('基础设置','eshop'), array(&$this, 'tax_box'), $this->pagehook, 'normal', 'core');
		add_meta_box('eshop-downloads-only', __('税率设置','eshop'), array(&$this, 'tax_box_2'), $this->pagehook, 'normal', 'core');
		add_filter('eshop_error_messages',array(&$this, 'eclass_errors'));
		add_screen_option('layout_columns', array('max' => 2, 'default' => 2) );
	}
	function taxenable_box($eshopoptions) {
		...
		?>
		<fieldset><br />
		<label for="eshop_tax"><?php _e('启用进出口税','eshop'); ?></label>
	...
	function tax_box($eshopoptions) {
		...
		?>
		<fieldset><br />
			<label for="eshop_tax_zonal"><?php _e('通过 (Shipping) Zones 计算税率','eshop'); ?></label>
			...
			<label for="eshop_tax_bands"><?php _e('税率模块数量','eshop'); ?></label><input type="text" id="eshop_tax_bands" name="eshop_tax_bands" value="<?php echo $etax['bands']; ?>" size="4" />
			<?php if( $etax['bands'] > 0 ) : ?>
			<label for="eshop_tax_shipping"><?php _e('税率编辑显示','eshop'); ?></label>
			<select name="eshop_tax_shipping" id="eshop_tax_shipping">
				<option value="" <?php selected($etax['shipping'],''); ?>><?php _e('发布产品时选择税率模块','eshop'); ?></option>
				<?php
				for($x=1;$x <= $etax['bands']; $x++){
					$tzone=sprintf(__('默认选择：税率模块 %1$d','eshop'),$x);
			...
			<label for="eshop_tax_unknown"><?php _e('税率的产品百分比 ?% (仅对下载产品类商城支持，如销售CDkey,软件等)','eshop'); ?></label><input type="text" id="eshop_tax_unknown" name="eshop_tax_unknown" value="<?php echo $etax['unknown']; ?>" size="4" />
			<?php endif; ?>
		</fieldset>
	<?php
	}
	function tax_box_2($eshopoptions) {
		...
		if($etax['bands']!='0'){
			...
			for($i=1;$i<=2;$i++){
				if($i==1){
					$area='state';
					echo '<h4>'.__('本地出口税','eshop').'</h4>';
				}else{
					$area='country';
					echo '<h4>'.__('收货国家进口税','eshop').'</h4>';
				}
				...
				<table class="hidealllabels widefat">
					<thead>
					<tr>
						<th id="<?php echo $eshopletter; ?>tax"><?php _e('税率模块','eshop'); ?></th>
					<?php
					if($etax['zonal']=='1'){
					...
					foreach ($query as $row){
						...
						$alt = ($x % 2) ? '' : ' class="alt"';
						echo '<tr'.$alt.'>';
						$tzone=sprintf(__('税率模块 %1$d (Band %1$d)','eshop'),$x);
						$disptzone=apply_filters('eshop_rename_tax_zone',array());
						if(isset($disptzone[$x]))
							$tzone=$disptzone[$x];
		...
		<?php
			...
			echo '<p>'.__('请设置税率模块数量后显示','eshop').'</p>';
		}
	}
