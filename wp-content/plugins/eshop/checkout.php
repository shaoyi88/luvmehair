...
if (!function_exists('eshopShowform')) {
	...
	global $wpdb, $blog_id,$eshopoptions,$eshop;
	...
	$echo = '
	<div class="eshopcustdetails custdetails" style="margin-top:-20px;" >
	<p><small class="privacy"><span style="float:right;color:red;"  class="reqd" title="Asterisk">* '.__('Denotes Required Field ','eshop').'</span>
	</small></p><div style="clear:both;"></div>
	...
	if($eshopoptions['shipping']=='4' && 'no' == $eshopoptions['downloads_only'] && !eshop_only_downloads()){
		...
		$echo .='';
		//2015-08-26 支付方式排除
		$eshopcart_data = $_SESSION['eshopcart'.$blog_id];
		$not_eshop_shiptype_arr=array();
		foreach($eshopcart_data as $key=>$val){
			$not_eshop_shiptype = get_post_meta( $val['postid'], '_not_eshop_shiptype');
			$not_eshop_shiptype_arr=array_merge($not_eshop_shiptype_arr,$not_eshop_shiptype[0]);
		}
		$not_eshop_shiptype_arr = array_unique($not_eshop_shiptype_arr);
		foreach($not_eshop_shiptype_arr as $v){
			unset($typearr[$v]);
		}
		//end 2015-08-26 支付方式排除
		...
		foreach ($typearr as $k=>$type){
			...
			if($query['0']->maxweight!='' && $cartweight > $query['0']->maxweight)
				continue;
			$eshopshiptableinner ='
			<div style="display:none;"><table class="eshopshiprates eshop">
			<thead>
			<tr>';
			...
			$eshopletter++;
			$eshopshiptableinner.='</tbody></table></div>'."\n";
			
			if($row->area=='country')
				$eshopshiptableheadtext = sprintf( __('%1$s ','eshop'),stripslashes(esc_attr($type)), __('(Shipping Zones by Country)','eshop'));
			else
				$eshopshiptableheadtext = sprintf( __('%1$s ','eshop'),stripslashes(esc_attr($type)), __('(Shipping Zones by State/County/Province)','eshop'));
			...
			$eshopshiptablehead='<div class="eshop_shiptype"><input class="rad" type="radio" name="eshop_shiptype" value="'.$k.'" id="eshop_shiptype'.$k.'"'.checked($stype,$k,false).' /></div><div class="eshop_shiptype_lable"><label for="eshop_shiptype'.$k.'">'.$eshopshiptableheadtext.'</label></div><div style="clear:both;"></div>';
			
			$eshopshiptable .= $eshopshiptablehead.$eshopshiptableinner;
			$js_eshopship.="eshop_shiptype_name[".$k."]='".trim($eshopshiptableheadtext)."';";
	...
	<fieldset class="eshop fld2">';
		...
		$echo .='<legend>'.__('Shipping Address','eshop').'</legend>';

	$echo .='<span class="email"><label for="email">'.__('Email','eshop').eshop_checkreqd($reqdarray,'email').'</label>
	  <input class="med" type="text" name="email" value="'.$email.'" id="email" maxlength="100" size="40" /><br /></span><span class="firstname"><label for="first_name">'.__('First Name','eshop').eshop_checkreqd($reqdarray,'first_name').'</label>
	 ...
	 <span class="lastname"><label for="last_name">'.__('Last Name','eshop').eshop_checkreqd($reqdarray,'last_name').'</label>
	  ...
	  <span class="phone"><label for="phone">'.__('Phone','eshop').eshop_checkreqd($reqdarray,'phone').'</label>
...
';
		...
		if($eshopoptions['show_allstates'] != '1'){
			...
			$echo .='<span class="state"><label for="state">State/County/Province</label>
			  <input class="med" type="text" name="state" value="'.$state.'" id="state" maxlength="100" size="40" /><br /></span>';
	...
	$echo.= '<style type="text/css" title="">.Shipping strong{margin:0px 5px;color:red}</style><span class="Shipping" id="shipping_box" style="display:none"><span id="Shipping">Ship to<strong id="s_country"></strong>Use <strong id="s_shopping_type"></strong>,Cost:<strong id="s_cost"></strong>.</span></span>';
	$echo .="</fieldset>";
	...
	if('yes' != $eshopoptions['hide_addinfo']){
		...
		<legend>'.__('Additional information','eshop').'</legend>
		 ...
		 <label for="eshop-comments">'.__('Comments or special instructions','eshop').eshop_checkreqd($reqdarray,'comments').'</label><br />
		  <textarea class="textbox" name="comments" id="eshop-comments" cols="80" rows="3">'.$comments.'</textarea>';
	...
	if('no' == $eshopoptions['downloads_only']){
		if('yes' != $eshopoptions['hide_shipping']){
			$echo .='<div class="checkout-item"><div style="display:none;">
<span class="checkout-trigger">if different,Click here</span><fieldset class="eshop fld4">
			<legend>'.__('Shipping address (if different)','eshop').'</legend><div class="checkout-cont">
	...
	if(is_array($eshopoptions['method'])){
		...
		if(sizeof((array)$eshopoptions['method'])!=1){
			foreach($eshopoptions['method'] as $k=>$eshoppayment){
				...
				$payfee_txt ='';
				if($eshopoptions[$eshoppayment]['payfee']&&$eshopoptions[$eshoppayment]['payfee']!=0){
					$payfee_txt = "Choose <b style='color:red'>".$eshoppayment."</b> will get <b style='color:red'> ".$eshopoptions[$eshoppayment]['payfee'].'%</b> more ';
				}else{
					$payfee_txt ='';
				}
               //
                $hide = '';
				if($_SESSION['final_price1']>$eshopoptions[$eshoppayment]['paylimit']&&intval($eshopoptions[$eshoppayment]['paylimit'])!=0){
					$hide = 'display:none;';
				}else{
					$hide = '';
				}

				$echo .='<div class="payvia_li"><li><input class="rad" type="radio" name="eshop_payment" value="'.$eshoppayment.'" id="eshop_payment'.$i.'"'.checked($eshop_paymentx,$eshoppayment,false).' /></li><label for="eshop_payment'.$i.'"><img style="padding:0px;border:0px solid #dadada;box-shadow:0 0 3px rgba(0,0,0,0.2);display:inline;" src="'.$eshopmerchantimgurl.'" '.$dims[3].' alt="'.__('Pay via','eshop').' '.$eshoppayment_text.'" title="'.__('Pay via','eshop').' '.$eshoppayment_text.'" /> '.$payfee_txt.'</label></div>'."\n";
				$i++;
			}
		}else{
foreach($eshopoptions['method'] as $k=>$eshoppayment){
               ...
               //
				...
				$echo .='<li style="padding:15px 0; '.$hide.'"><input class="rad" type="radio" name="eshop_payment" value="'.$eshoppayment.'" id="eshop_payment'.$i.'"'.checked($eshop_paymentx,$eshoppayment,false).' /><label for="eshop_payment'.$i.'"><img style="padding:0px;border:0px solid #dadada;box-shadow:0 0 3px rgba(0,0,0,0.2);display:inline;" src="'.$eshopmerchantimgurl.'" '.$dims[3].' alt="'.__('Pay via','eshop').' '.$eshoppayment_text.'" title="'.__('Pay via','eshop').' '.$eshoppayment_text.'" /> '.$payfee_txt.'</label></li>'."\n";
				$i++;}
		}
		$echo .="</ul>\n";
	...
	$echo .='</div></fieldset><span class="buttonwrap" style="float:right;"><input style="width:180px;" type="submit" class="button" id="submitit" name="submit" value="'.__('Proceed to Confirmation &raquo;','eshop').'" /></span>
	</div>
	</form>
	</div>';
	$echo .='<input type="hidden" id="shiptype" name="shiptype" value="1"/>
<input type="hidden" id="yes_country" name="yes_country" value="0"/>
<script>
function show_shopping() {
		  var ship_name = $(\'#ship_name\').val();
		  var eshop_shiptype = $(\'#shiptype\').val();
		  var country = $("#country").find(\'option:selected\');
		  var ship_country = country.val();
		  var ship_country_name = country.text();		  
		  var eshop_shiptype_name=new Array();
		  '.$js_eshopship.'  
		  $.post("/wp-admin/admin-ajax.php?action=get_shipping_amt", { ship_name: ship_name, eshop_shiptype: eshop_shiptype, ship_country:ship_country },
		  function(data){
			    if(data!=0){
					$(\'#s_country\').html(ship_country_name);
					$(\'#s_shopping_type\').html(eshop_shiptype_name[eshop_shiptype]);
					$(\'#s_cost\').html(data);	
					$(\'#shipping_box\').show();	
				}
		});		
...
$(document).ready(function(){
	$(\'input[name=eshop_shiptype]\').click(function(){
		$(\'#shiptype\').val($(this).val());
		var yes_country=$(\'#yes_country\').val();
		if(yes_country==1){
			show_shopping();
		}
	 })
...
if (!function_exists('eshop_checkout')) {
 	function eshop_checkout($espost){
		...
		if((isset($_GET['eshopaction']) && $_GET['eshopaction']!='redirect')||!isset($_GET['eshopaction'])){
			$echoit.= display_cart($_SESSION['eshopcart'.$blog_id], false,$eshopoptions['checkout'],$pzone,$shiparray, $espost);
		}
	}
	$error='';
	$dtable=$wpdb->prefix.'eshop_rates';
	...
	if (isset ($espost['submit'])) {
		...
		if(!isset($espost['eshop_payment'])){
			$error.= '<li>'.__('You have not chosen a <strong style="color:#FF0000;">payment option</strong>.','eshop').'</li>';
		}
		if(eshop_discount_codes_check()){
			$_SESSION['eshop_discount'.$blog_id]='';
			unset($_SESSION['eshop_discount'.$blog_id]);
		...
		if($error!=''){
			...
			if(!isset($_GET['eshopaction'])){
				...
				if('no' == $eshopoptions['downloads_only']){
					$echoit.='<h4>'.__('Shipping Address','eshop').'</h4><br /><ul class="eshop confirm">';
				}else{
					$echoit.='<h4>'.__('Contact Details','eshop').'</h4><ul class="eshop confirm">';
				}
				$echoit.= "<br />
				<li class=\"email\"><span class=\"items\">".__('Full name:','eshop')."</span> ".$espost['first_name']." ".$espost['last_name']."</li>\n
";
				if('no' == $eshopoptions['downloads_only']){
					$echoit.= "";
				}
				...
				if('no' == $eshopoptions['downloads_only']){
					...
					$echoit.= "<li class=\"state\"><span class=\"items\">".__('State/County/Province:','eshop')."</span> ".$espost['state']."</li>\n";
				...
				if('no' == $eshopoptions['downloads_only']){
					if('yes' != $eshopoptions['hide_shipping']){
						if($espost['ship_name']!='' || $espost['ship_address']!='' || $espost['ship_city']!='' || $espost['ship_postcode']!=''){
							$echoit.= "<div style=\"display:none;\" class=\"eshop fld4\"><h4>".__('Shipping Address','eshop')."<br /></h4>\n<ul class=\"eshop confirmship\">\n";
						    ...
						    $echoit.= "<li class=\"state\"><span class=\"items\">".__('State/County/Province:','eshop')."</span> ".$state."</li>\n";
			...
			$eshop_plugin_settings = get_option('eshop_plugin_settings');
			if($eshop_plugin_settings[$espost[eshop_payment]]['payfee']!=''){
				for ($i = 1; $i <= $espost['numberofproducts']; $i++) {
					$espost['amount_'.$i] = $espost['amount_'.$i]+$espost['amount_'.$i]*($eshop_plugin_settings[$espost[eshop_payment]]['payfee']/100);
				}
			}     			
			$eshopmgincpath=apply_filters('eshop_mg_inc_path',ESHOP_PATH.$paymentmethod.'.php',$paymentmethod);
		...
		if(isset($_SESSION['addy'.$blog_id])){
			...
			if(isset($_COOKIE["eshopcart"]) && calculate_items()!=0 && $eshopsetcookie==true){
				...
				global $current_user;
				get_currentuserinfo();
				$crumbs=array('first_name'=>"first_name",'last_name'=>"last_name",'company'=>"company",'phone'=>"phone",'email'=>"user_email",'address1'=>"add1",'address2'=>"add2",'city'=>"city",'state'=>"state",'altstate'=>"altstate",'zip'=>"zip");

				foreach($crumbs as $k=>$v){
						$$k=esc_attr($current_user->$v);
				}			
			}
	...
	if(isset($_SESSION['eshopcart'.$blog_id])){
		switch($contineproceed){
			case ('1'):
				$echoit.='<div style="clear:both;"></div><ul style="margin:-50px 0 0 20px;" class="continue-proceed eshopcp1"><li class="editcart"><a href="'.get_permalink($eshopoptions['cart']).'">'.__('&laquo; Edit Cart or Continue Shopping','eshop').'</a></li></ul>';
				break;
			case ('2'):
				$echoit.='<div style="clear:both;"></div><ul class="continue-proceed redirect eshopcp2"><li class="editcheckout"><a href="'.get_permalink($eshopoptions['checkout']).'">'.__('&laquo; Edit Details or Continue Shopping','eshop').'</a></li></ul>';
				break;
			case ('3'):
				$echoit.='<div style="clear:both;"></div><ul class="continue-proceed redirect eshopcp3"><li class="editcheckout"><a href="'.get_permalink($eshopoptions['checkout']).'">'.__('&laquo; Edit Details or Continue Shopping','eshop').'</a></li></ul>';
				break;
		}
