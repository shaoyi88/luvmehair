...
echo '<div class="wrap"><div id="eshopicon" class="icon32"></div><h2>'.__('GD-Shop Uninstall','eshop').'</h2>';

if(isset($_POST['delete'])){
	echo '<h3>'.__('Confirm uninstall of GD-Shop','eshop').'</h3>';
	...
	if ($handle = opendir($filedir[0])) {
		...
		closedir($handle);
		rmdir ($filedir[0]);
		echo '<li>'.__('GD-Shop template files deleted','eshop').'</li>';
	}
	//unregister widgets
	...
	echo '</ul>';
	echo '<p><strong>'.__('GD-Shop uninstalled.','eshop').'</strong></>';

}else{
	echo '<p><strong>'.__('Uninstalling GD-Shop will result in the following:','eshop').'</strong></p>';
	echo '<ul>';
	...
	echo '<li>'.__('Deactivation and removal of GD-Shop widgets.','eshop').'</li>';
