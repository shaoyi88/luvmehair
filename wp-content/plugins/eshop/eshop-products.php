...
function eshop_products_manager() {
	...
	if(current_user_can('eShop_admin')){
		...
		?>
		<div class="wrap">
		<div id="eshopicon" class="icon32"></div><h2><?php _e('商品批 量查看','eshop'); ?></h2>
		<?php eshop_admin_mode(); ?>
		<h3><?php _e('按网站编辑账户查看','eshop'); ?></h3>
		<?php if(isset($msg)) echo '<div class="updated fade"><p>'.$msg.'</p></div>'; ?>
		<div style="float:left;">		
		<form action="" method="post" class="eshop filtering">
		<p><label for="eshopfilter"><?php _e('显示产品：','eshop'); ?></label><select name="eshopfilter" id="eshopfilter">
		<?php
		echo eshop_authors($eshopfilter);
		?>
		</select> <input type="submit" name="eshopfiltering" id="submit"  class="submit button-primary" value="过滤" /></p>
		</form>
		</div>
		<div style="float:right;">
			<form action="" method="post" class="eshop filtering">
			<p><label for="p_filter"><?php _e('订单搜索：','eshop'); ?></label>
			<select name="s_type" id="s_type">
			<option value="1" <?php echo $s1;?>>产品SKU</option>
			<option value="2" <?php echo $s2;?>>产品ID</option>
			<option value="3" <?php echo $s3;?>>产品标题</option>
			</select> <input type="text" name="s_q" value="<?php echo $s_q;?>"/><input type="submit" name="eshopso" id="submit"  class="submit button-primary" value="搜索" /></p>
			</form>
		</div>
	...
	if(current_user_can('eShop_admin')){
		if($s_type==1)
			$addtoq="AND meta.meta_value like '%$s_q%'";
		elseif($s_type==2)
			$addtoq="AND posts.ID = $s_q";
		elseif($s_type==3)
			$addtoq="AND posts.post_title like '%$s_q%'";
	}
	...
	if($max>0){
		...
		echo '<li><span>'.__('排序方法 &raquo;','eshop').'</span></li>';
		echo '<li> <a href="'.$apge.'&amp;by=sf"'.$csf.'>'.__('产品ID','eshop').'</a></li>';
		// echo '<li><a href="'.$apge.'&amp;by=sa"'.$csa.'>'.__('产品Sku','eshop').'</a></li>';
		echo '<li><a href="'.$apge.'&amp;by=sb"'.$csb.'>'.__('产品名称','eshop').'</a></li>';
		// echo '<li><a href="'.$apge.'&amp;by=sc"'.$csc.'>'.__('Shipping','eshop').'</a></li>';
		echo '<li><a href="'.$apge.'&amp;by=sd"'.$csd.'>'.__('库存','eshop').'</a></li>';
	...
	?>	
		...
		<div class="eshopwidetable"><div class="eshopwidetable">
		<table class="hidealllabels widefat">
		<thead>
		<tr>
		<!--<th id="sku"><?php _e('产品Sku','eshop'); ?></th>-->
		<th id="ids"><?php _e('产品ID(系统自动生成)','eshop'); ?></th>
		<th id="opt"><?php _e('产品SKU','eshop'); ?></th>
			<th id="page"><?php _e('产品标题 - 点击进入产品编辑页面','eshop'); ?></th>
		<th id="stkavail"><abbr title="<?php _e('Stock Available','eshop'); ?>"><?php _e('有库存?','eshop'); ?></abbr></th>
		<th id="purc"><abbr title="<?php _e('Number of Purchases','eshop'); ?>"><?php _e('销量','eshop'); ?></abbr></th>
		<th id="onsale" title="<?php _e('Product on sale','eshop'); ?>"><?php _e('销售中？','eshop'); ?></th>
		<th id="opt"><?php _e('产品卖价','eshop'); ?></th>

		<?php if ( $etax['bands'] > 0 ) : ?>
		<th id="tax"><?php _e('Tax','eshop'); ?></th>
		<?php endif; ?>
		<th id="associmg"><?php _e('缩略图','eshop'); ?></th>
		</tr>
		</thead>
		<tbody>
		<?php
		...
		foreach($grab as $grabit){
			...
			//reset array
				...
				for($i=1;$i<=$numoptions;$i++){
					if($eshop_product['products'][$i]['option']!=''){
						echo sprintf( __('<td headers="opt sku'.$calt.'" class="optline">%1$s','eshop'),stripslashes(esc_attr($eshop_product['products'][$i]['option'])), $currsymbol, number_format_i18n($eshop_product['products'][$i]['saleprice'],__('2','eshop'))).'<br />';
					}
				}
				echo '</td>';
				...
				for($i=1;$i<=$numoptions;$i++){
					if($eshop_product['products'][$i]['option']!=''){
						echo sprintf( __('%2$s%3$s</td>','eshop'),stripslashes(esc_attr($eshop_product['products'][$i]['option'])), $currsymbol, number_format_i18n($eshop_product['products'][$i]['saleprice'],__('2','eshop'))).'';
					}
				}
				echo '';
				if ( $etax['bands'] > 0 ) {
				...
				if (has_post_thumbnail( $getid ) ) {
					 echo '<div class="eshoppic"><a class="itemref" href="'.get_permalink($getid).'" title="view page">'.get_the_post_thumbnail( $getid, array($w, $h)).'</a></div>'."\n";
				}else{
		...
		echo '<div class="paginate tablenav-pages stuffbox">';
			...
			echo sprintf( '<span class="displaying-num">' . __( '产品显示 %s&#8211;%s of %s', 'eshop' ) . '</span>',
				$eshopdisp,
