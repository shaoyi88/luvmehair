...
echo '<div class="wrap">';
echo '<h2>'.__('GD-Shop 邮件发送系统','eshop').'</h2><p>'.__('系统会自动获取客户订单信息！','eshop').'</p>';
...
if(isset($_POST['thisemail']) && isset($_GET['viewemail'])){
	...
	echo '<p class="success">'.__('邮件发送成功.','eshop').'</p>';
	echo '<p><a class="return" href="'.$page.'">'.__('&laquo; 返回订单详情','eshop').'</a></p>';
...
// For system email - 
	...
	<fieldset><legend><?php _e('发送邮件至: ','eshop'); ?> <strong><?php echo $email; ?></strong></legend><br />
	<label for="from"><?php _e('选择显示发送邮箱(必须和SMTP设置的邮箱相同，否则会失败): ','eshop'); ?><br />
	...
	?></select></label><br /><br />
	<label for="subject"><?php _e('邮件标题:','eshop'); ?><br /><input type="text" id="subject" name="subject" size="60" value="<?php echo $subject; ?>" /></label><br />
	<br /><label for="thisemail"><?php _e('邮件内容(支持邮件HTML源码，具体代码规则，请百度！):','eshop'); ?><br /><textarea name="thisemail" id="thisemail" cols="70" rows="20"><?php echo $this_email; ?></textarea></label>
