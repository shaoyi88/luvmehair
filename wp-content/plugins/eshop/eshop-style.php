...
?>
<div class="wrap">
<div id="eshopicon" class="icon32"></div><h2><?php _e('GD-Shop Styles','eshop'); ?></h2>
...
?></textarea>
   <p class="submit eshop"><input type="submit" class="button-primary" value="<?php _e('Update Style','eshop'); ?>" name="submit" /></p>
	...
	if(isset($_GET['diff'])){
		...
		echo wp_text_diff( $right_string,$left_string, array( 'title' => __('Comparing Current Style with latest installed version of GD-Shop','eshop'), 'title_right' => __('Latest(from plugin)','eshop'), 'title_left' => __('Current (in use)','eshop') ) );
		echo '</div>';
	}elseif(trim($left_string)!=trim($right_string)){
		echo '<div class="wrap">';
		echo '<p>'.__('There may have been updates to the style.','eshop').' <a href="themes.php?page=eshop-style.php&amp;diff#diff">'.__('Compare Current Style with latest installed version of GD-Shop.','eshop').'</a></p>';
		echo '</div>';
	}else{
		echo '<div class="wrap">';
		echo '<p>'.__('Your CSS matches that included with GD-Shop.','eshop').'</p>';
		echo '</div>';
	}
}
?>