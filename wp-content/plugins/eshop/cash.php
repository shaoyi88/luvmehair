...
switch ($eshopaction) {
    case 'redirect':
		...
		$subject .=__(": ",'eshop').$ecash->ipn_data['RefNr'];
		...
		$ecash->ipn_data['payer_email']=stripslashes($array['ename']).' '.$array['eemail'].' ';
		 $body =  __("选择银行付款订单（线下付款方式）",'eshop')."\n";
		 $body .= "\n".__("来自 ",'eshop').$ecash->ipn_data['payer_email'].__("； 订单时间：",'eshop').date('m/d/Y');
